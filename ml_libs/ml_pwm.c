...
void pwm_pin_init(GPTIMER_Regs *timgn, DL_TIMER_CC_INDEX ch, uint16_t fre, uint8_t *psc, uint32_t *arr)
{
	...
	else if(timgn == TIMG8)
	{
		if(ch == DL_TIMER_CC_0_INDEX)
		{
			DL_GPIO_initPeripheralOutputFunction(IOMUX_PINCM32,IOMUX_PINCM32_PF_TIMG8_CCP0);
		...
		{
			DL_GPIO_initPeripheralOutputFunction(IOMUX_PINCM33,IOMUX_PINCM33_PF_TIMG8_CCP1);
			DL_GPIO_enableOutput(GPIOB, DL_GPIO_PIN_16);
			*psc = 9;
			*arr = 500000 / fre - 1;
		}
	}
